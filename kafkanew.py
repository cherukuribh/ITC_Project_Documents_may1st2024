...
formattedDF = streamingDF \
    .select(
        splitCols.getItem(0).cast(IntegerType()).alias("Account_ID"),
        splitCols.getItem(1).cast(DoubleType()).alias("transaction_Amount"),
        splitCols.getItem(2).cast(StringType()).alias("Transaction_Status"),
...
failed_transactions_df = formattedDF \
    .filter(f.col("Transaction_Status") == "Failed") \
...
query = failed_transactions_df \
     .writeStream \
     .format("parquet") \
     .option("path", "/user/ec2-user/UKUSMarHDFS/bharathi/newkafka") \
     .option("checkpointLocation", "/user/ec2-user/UKUSMarHDFS/bharathi/newoffset") \
     .outputMode("append") \
     .start()
